        ...
        try {
        	...
        	try {
        		...
        		if (PortletFileUpload.isMultipartContent(newRequest.getAsActionRequest())) {
        			System.err.println("Got a Multipart Resource Request (Content-Type: '" + request.getContentType() + "')");
        			...
        			if (result != null && !result.isEmpty()) {
        				for (FileItem item : result) {
        					if (item.isFormField()) {
        						System.err.println("   Form Field .. '" + item.getFieldName() + "'");// = '" + item.getString().substring(0, 10) + "'");
        						parsedParameters.put(item.getFieldName(), new String[] { item.getString() });
        					}
        					else {
        						System.err.println("   Attribute ... '" + item.getFieldName() + "'");// = '" + item.getString() + "'");
        						...
        						DiskFileItem diskFileItem = (DiskFileItem) diskFileItemFactory.createItem(item.getFieldName(), item.getContentType(), item.isFormField(), item.getName());
								...
								UploadedFile uploadedFile = new UploadedFileImpl(
										tempFile.getAbsolutePath(), attributeMap, diskFileItem.getCharSet(),
										diskFileItem.getContentType(), headersMap, id, message, item.getName(),
